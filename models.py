...
class WeightSumLayer(torch.nn.Module):
    def __init__(self, n_layers = 13):
        super(WeightSumLayer, self).__init__()
        self.constant_parameter = nn.Parameter(torch.randn(size=(1, n_layers, 1, 1), requires_grad=True))
    def forward(self, x):
        x = torch.mul(x, self.constant_parameter)
        x = torch.sum(x, dim=1)
        return x

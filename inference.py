...
def inference(model_ckpt_path, model_config_path, pl_bert_dir):
    ...
    with torch.inference_mode():
        ...
        # get bert features
        bert_features = get_bert_features(dummy_text, pl_bert_model, pl_bert_tokenizer, pl_bert_config, pl_bert_cleaner).to(device)
        x = get_text_ids(dummy_text, hps.cleaner_names)
        x = x.to(device).unsqueeze(0)
        bert_features = bert_features.to(device).unsqueeze(0)
        x_lengths = torch.LongTensor([x.size(0)]).to(device)
        sid = torch.zeros(size=(0,)).to(device)
        net_g.module.infer(x, x_lengths, bert_features, x_lengths, sid,
    ...
    while True:
        ...
        with torch.inference_mode():
            ...
            x_lengths = torch.LongTensor([x.size(0)]).to(device)
            sid = torch.zeros(size=(0,)).to(device)
            y_hat, _, _, _ = net_g.module.infer(x, x_lengths, bert_features, x_lengths, sid,
                                                noise_scale=noise_scale, 
                                                noise_scale_w=noise_scale_w, 
                                                length_scale=length_scale,
                                                max_len=1000)
            y_hat = y_hat[0, :, :].cpu().float().numpy().copy()
...
def get_text_ids(self, text, text_cleaners):
...
def get_phonemes(text, pl_bert_tokenizer):
    return ''.join(phonemize(text,pl_bert_tokenizer)["phonemes"])

def get_bert_features(text, pl_bert_model, pl_bert_tokenizer, pl_bert_config, pl_bert_cleaner, ):
    text = text.replace("\n", "")
    hidden_size = pl_bert_config["model_params"]["hidden_size"]
    n_layers = pl_bert_config["model_params"]["num_hidden_layers"] + 1
    phonemes = get_phonemes(text, pl_bert_tokenizer)
    input_ids = pl_bert_cleaner(phonemes)
    with torch.inference_mode():
        hidden_stats  = pl_bert_model(torch.tensor(input_ids, dtype=torch.int64).unsqueeze(0))[-1]["hidden_states"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_ckpt_path", default="./logs/test/G_1000.pth")
    parser.add_argument("--model_cnfg_path", default="./logs/test/config.json")
    parser.add_argument("--pl_bert_dir", default="./plb-ja_10000000-steps/")

    args = parser.parse_args()

    inference(args.model_ckpt_path, args.model_cnfg_path, args.pl_bert_dir)